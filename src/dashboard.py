...
def stream_windows():
    ...
    for i, (features, label) in enumerate(zip(X, y)):
        ...
        controller.process_window(features, timestamp)
        stream_index = i
        time.sleep(0.05)
    streaming_done = True
...
def api_alerts():
    return jsonify(controller.get_alert_history())

@app.route('/api/history')
def api_history():
    return jsonify(controller.alert_log[-50:])
...
def api_stream():
    def generate():
        last_sent = 0
        while True:
            current = len(controller.alert_log)
            if current > last_sent:
                entry = controller.alert_log[-1]
...
if __name__ == '__main__':
    print("\n✅ SDN Controller loaded")
    print("🌐 Dashboard running at http://localhost:5000")
    print("📡 Streaming 717 windows in background...")
    print("🔴 Press Ctrl+C to stop\n")
    threading.Timer(1.5, lambda: subprocess.Popen(['open', '-a', 'Google Chrome', 'http://localhost:8080'])).start()
